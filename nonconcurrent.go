...
func main() {
	...
	for i := 0; i < len(URLS); i++ {
		CSVs = append(CSVs, runCSVJobs(URLS[i], startTime))
	}
...
func printStats(sortedCSV [][]string, startTime time.Time) {
	printMedian(sortedCSV, startTime)
	printMean(sortedCSV, startTime)
}

// Finds median of CSV
func printMedian(csv [][]string, startTime time.Time) {
	func_time := time.Now()
	csv = csv[1:]
	var median float64
	l := len(csv)
...
// Prints mean
func printMean(csv [][]string, startTime time.Time) {
	func_time := time.Now()
	result := 0
	var newNum int
	var mean float64
